	...
	public class RedisConnectionManager : IRedisConnectionManager, IDisposable
	{
		...
		private Lazy<ConnectionMultiplexer> CreateConnection()
		{
			return new Lazy<ConnectionMultiplexer>(() =>
			{
				...
				// Keep the connection alive to prevent idle TCP resets from intermediaries
				configOptions.KeepAlive = 60;
				// If the connection string didn't include the host as SSL target, prefer setting it explicitly
				if (configOptions.EndPoints.Count > 0 && string.IsNullOrEmpty(configOptions.SslHost))
				{
					var ep = configOptions.EndPoints[0] as DnsEndPoint;
					if (ep != null)
					{
						configOptions.SslHost = ep.Host;
					}
				}
